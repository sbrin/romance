...
7. При перезагрузке/переподключении по нажатию «начать поиск» снова вызвать
   `POST /queue/join` — сервер вернет `PARTNER_FOUND`, если пара уже создана.
8. (UI) При отмене поиска клиент вызывает `POST /queue/cancel` и возвращается
   к экрану «начать поиск» без повторного выбора роли.
9. (UI) Если отмена происходит после `partner_found` (до старта сессии),
