...
4. (UI) Сохранить роль локально и использовать ее для повторных запусков поиска
   без повторного выбора.
5. (UI) Показать экран «начать поиск»; по нажатию вызвать `POST /queue/join`.
6. Ждать событие `partner_found`. Если `POST /queue/join` сразу вернул `PARTNER_FOUND`,
   можно считать мэтч полученным.
7. Показать кнопку «Начать» на экране мэтча и отправить `POST /session/start` после
   подтверждения пользователя.
8. Если второй пользователь ещё не подтвердил старт — показать статус «Жду партнера».
9. Когда оба подтвердили старт, сервер отправляет событие `session_started`.
10. При перезагрузке/переподключении по нажатию «начать поиск» снова вызвать
   `POST /queue/join` — сервер вернет `PARTNER_FOUND`, если пара уже создана.
11. (UI) При отмене поиска клиент вызывает `POST /queue/cancel` и возвращается
