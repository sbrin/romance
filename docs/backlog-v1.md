...
- [x] Клиент рендерит `<video>` (loop, muted, playsinline) из URL текущего шага.
- [x] Показывается текстовый "бабл" (контекст шага).
- [x] Активный игрок видит кнопки выбора ответа.
- [x] Пассивный игрок видит статус "Партнер выбирает..." и скрытые кнопки.
- [x] Если у шага нет видео для роли пользователя, видео остается прежним.

**Notes:**

- Tech: состояние `stepId` (id текущего шага) и `turnDeviceId` приходит с
  бэкенда. Формат названий — только `camelCase`. Видео шага задается отдельно
  для М/Ж; если пусто — не менять видео у этой роли.
...
- [x] Нажатие кнопки отправляет `POST /session/step/answer`.
- [x] `POST /session/step/answer` содержит `choiceId` (id следующего шага).
- [x] Оба клиента получают обновление `session_step`.
- [x] У обоих отображается бабл с текстом следующего шага.
- [x] У партнера (на кого смотрели) видео меняется на новое (реакция или след.
      шаг).
- [x] У того, кто выбирал, видео остается (или меняется по сценарию).
- [x] Выбор разрешен только `turnDeviceId`; иначе сервер отвечает `NOOP`.

**Notes:**

- Core mechanic: выбор влияет на видео партнера.
- `choiceId` валидируется на сервере по `currentStep.choices`.
- Шаги переходят через `session_step` (отдельного `step_advanced` нет).
...
- [x] После завершения показывается экран «Сессия завершена».
- [x] Кнопка «В очередь» возвращает в состояние ожидания.
- [x] Кнопка «В очередь» вызывает `POST /session/end`, затем `POST /queue/join`.
- [x] Состояние шага сбрасывается автоматически при завершении.
- [x] Терминальный шаг = нет `choices`; сервер шлет `session_ended` (reason: completed).
...
- [ ] Список событий: `selected_gender`, `queued`, `partner_found`,
      `start_pressed`, `session_started`, `choice_made`, `step_shown`,
