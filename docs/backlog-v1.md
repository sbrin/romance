...
- P0-01 Выбрать роль и сохранить выбор
- P0-02 Встать в очередь и видеть ожидание
- P0-03 Получить мэтч и увидеть “Партнёр найден”
- P0-04 Стартовать сессию только после обоих
...
- [ ] При входе показывается экран с 2 кнопками: "Мужчина" и "Женщина".
- [ ] Выбор сохраняется в локальном состоянии и отправляется на сервер
      (`POST /role`).
- [ ] После выбора происходит переход на экран ожидания.
- [ ] Повторный заход показывает сохраненное состояние (не спрашивает заново).
...
- [ ] На экране мэтча есть кнопка "Start".
- [ ] Нажатие отправляет `POST /session/start`.
- [ ] Статус меняется на "Жду партнера", если второй еще не нажал.
- [ ] Когда оба нажали — сервер шлет `session_started` и первый шаг.
