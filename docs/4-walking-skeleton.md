...
- Экран «Вы в очереди» + индикатор ожидания + кнопка «Отмена».
- Экран «Партнёр найден» + кнопка Start + кнопка «Отмена» + статус «жду второго».
- Экран шага: Фулл-скрин видео (Loop) + одна реплика (бабл) + 2–3 кнопки
  ответа + состояния «мой ход / жду». Видео обновляется только если для роли
  есть параметры видео, иначе остаётся прежним.
...
- Один шаг диалога с выбором ответа (внутри `active` храним `stepId`,
  `turnDeviceId`).
- Синхронный переход на следующий (в skeleton можно завершать сразу после выбора
  обоими).
- Резолвинг контента: шаг содержит **раздельные видео для М/Ж**; если видео для
  роли отсутствует — сохраняется предыдущее.
