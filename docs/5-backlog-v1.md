...
- [ ] `POST /session/step/answer` содержит `choiceId` (id следующего шага).
- [ ] Оба клиента получают обновление `session_step`.
- [ ] У обоих отображается бабл с текстом следующего шага.
...
- [ ] Выбор разрешен только `turnDeviceId`; иначе сервер отвечает `NOOP`.
...
- [ ] Кнопка «В очередь» вызывает `POST /session/end`, затем `POST /queue/join`.
- [ ] Состояние шага сбрасывается автоматически при завершении.
- [ ] Терминальный шаг = нет `choices`; сервер шлет `session_ended` (reason: completed).
