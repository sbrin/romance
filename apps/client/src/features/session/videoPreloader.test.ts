...
test('videoPreloader exports required methods', () => {
  ...
  assert.equal(typeof videoPreloader.getVideo, 'function')
...
test('getVideo returns null for non-preloaded videos', () => {
  videoPreloader.clear()
  assert.equal(videoPreloader.getVideo('test-video.mp4'), null)
})
